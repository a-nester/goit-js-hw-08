...
// якщо воно задоволяє умову, мають додаватися нові < div > елементи в
// DOM.

// Для рендеру елементів на сторінці створи функцію createBoxes(amount),
// яка приймає один параметр — число, що зберігає кількість елементів 
// для рендеру.

// Функція має створювати стільки < div > елементів, скільки вказано в 
// параметрі amount і додавати їх у DOM дочірніми елементами для 
// div#boxes.
...
function createBoxes(amount) {
  ...
  for (let i = 0; i < amount; i++) {
    ...
    newElement.style.backgroundColor = divColor;
    newElement.style.width = `${width}px`;
    newElement.style.height = `${height}px`;
    arr.push(newElement.outerHTML);
    width += 10;
    height += 10;
  }
  return arr.join("");
